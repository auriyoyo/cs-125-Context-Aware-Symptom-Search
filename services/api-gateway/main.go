//go:build !ingest && !live_events_ingest
...
func main() {
	...
	*/
	...
	var leStore *liveevents.Store
	if database.Client != nil {
		db := database.GetDatabase("kaggle_dataset")
		pmStore = personalmodel.NewStore(db)

		leCfg := liveevents.LoadConfig()
		leDB := database.GetDatabase(leCfg.DatabaseName)
		leStore = liveevents.NewStore(leDB, leCfg)
		leSource := liveevents.NewSource(leStore)
		sources = append(sources, leSource)
	}
	...
	registerLiveEventHandlers(leStore)
	go startHTTP(store, pmStore, leStore)
